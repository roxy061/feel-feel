...
function GlobalTrackContent() {
  ...
  const getStepProgress = (order: TrackedOrder) => {
    ...
    if (order.shipping_status === "shipped") return 4;
    if (order.shipping_status === "packing") return 3;
    if (order.status === "paid" || order.payment_status === "paid") return 2;
    return 1;
  };
    ...
    <div className="min-h-[100dvh] bg-[#010101] text-[#EEEFF2] flex flex-col justify-between selection:bg-[#272835] selection:text-[#EEEFF2]">
      ...
      <header className="border-b border-[#EEEFF2]/10 bg-[#090A0F]/80 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-18 flex items-center justify-between">
          ...
          <Link
            href="/dashboard"
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-[#272835]/50 hover:bg-[#272835] border border-[#EEEFF2]/10 text-xs font-sans text-[#EEEFF2]/70 hover:text-white transition-colors"
          >
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-10 sm:py-14">
        <div className="text-center max-w-2xl mx-auto mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-xl bg-[#272835] border border-[#EEEFF2]/15 font-mono text-xs text-amber-400 mb-3">
            ...
            <span>UNIVERSAL TRACKING PORTAL</span>
          </div>
          <h2 className="font-bebas text-4xl sm:text-5xl tracking-wide text-[#EEEFF2]">
            ติดตามสถานะคำสั่งซื้อ & พัสดุ
          </h2>
          <p className="font-sans text-xs sm:text-sm text-[#EEEFF2]/65 mt-2 leading-relaxed">
            ค้นหาคำสั่งซื้อด้วยหมายเลขคำสั่งซื้อ (เช่น <span className="font-mono text-[#EEEFF2]">ORD-20260912-XXXXX</span>) หรือเบอร์โทรศัพท์
          </p>
        ...
        {hasSearched && !isLoading && orders.length === 0 && (
          <div className="rounded-xl bg-[#090A0F] border border-[#EEEFF2]/10 p-12 text-center max-w-lg mx-auto">
            ...
            <p className="font-sans text-xs text-[#EEEFF2]/60 mt-1 max-w-xs mx-auto leading-relaxed">
              ไม่พบคำสั่งซื้อที่ตรงกับ &ldquo;{query}&rdquo; กรุณาตรวจสอบข้อมูลอีกครั้ง
            </p>
          </div>
        )}
                ...
                >
                  ...
                  <div className="p-5 sm:p-6 bg-[#272835]/40 border-b border-[#EEEFF2]/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <div className="font-mono text-xs text-[#EEEFF2]/50 mt-1 flex items-center gap-2">
                        <span>สั่งซื้อเมื่อ: {new Date(order.created_at).toLocaleString("th-TH")}</span>
                        {order.store_name && (
                    ...
                    <div className="flex items-center gap-2">
                      {order.store_subdomain && (
                        <Link
                          href={`/${order.store_subdomain}`}
                          className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#272835]/60 hover:bg-[#272835] border border-[#EEEFF2]/10 text-xs font-sans text-[#EEEFF2] transition-colors"
                        >
